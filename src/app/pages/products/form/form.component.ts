...
export class FormComponent implements OnInit{
  ...
  async sumbit() {
    ...
    if (this.fileList.length > 0 &&(!this.data?.imageUrl || this.fileList[0].url !== this.data.imageUrl)) {
      imageUrl = await this.supabase.uploadImage(this.fileList[0]) || '';
    ...
    const res = this.data ? await this.supabase.updateProduct(this.data.id, params) : await this.supabase.addProduct(params);
